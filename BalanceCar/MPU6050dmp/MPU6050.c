						...
						int16_t *GyroX, int16_t *GyroY, int16_t *GyroZ)
{
	...
	DataH = mpu6050_read_reg(ACCEL_XOUT_H);
	DataL = mpu6050_read_reg(ACCEL_XOUT_L);
	*AccX = (DataH << 8) | DataL;
	
	DataH = mpu6050_read_reg(ACCEL_YOUT_H);
	DataL = mpu6050_read_reg(ACCEL_YOUT_L);
	*AccY = (DataH << 8) | DataL;
	
	DataH = mpu6050_read_reg(ACCEL_ZOUT_H);
	DataL = mpu6050_read_reg(ACCEL_ZOUT_L);
	*AccZ = (DataH << 8) | DataL;
	...
	*GyroX = (DataH << 8) | DataL;
	...
	DataH = mpu6050_read_reg(GYRO_YOUT_H);
	DataL = mpu6050_read_reg(GYRO_YOUT_L);
	*GyroY = (DataH << 8) | DataL;
